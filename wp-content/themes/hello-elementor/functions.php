...
function jg_cart_qty_for(int $target_id): int
{
	...
	foreach (WC()->cart->get_cart() as $ci) {
		...
		if ($pid === $target_id || $vid === $target_id) {
			$qty += (int) $ci['quantity'];
		}
	}
	return $qty;
	...
	foreach (jg_min_rules() as $id => $min) {
		...
		if ($qty > 0 && $qty < (int) $min) {
			...
			foreach (WC()->cart->get_cart() as $ci) {
				$pid = (int) $ci['product_id'];
				$vid = isset($ci['variation_id']) ? (int) $ci['variation_id'] : 0;
				if ($pid === (int) $id || $vid === (int) $id) {
					$name = $ci['data']->get_name();
					break;
				}
			}
			$blocked[] = ['id' => (int) $id, 'name' => $name, 'qty' => (int) $qty, 'min' => (int) $min];
		}
	}
	return $blocked;
}
...
// ---------------- CHECKOUT/CART VALIDATOR (single combined notice; run once) ----------------
add_action('woocommerce_check_cart_items', function () {
	static $ran = false;
	if ($ran)
		return;         // avoid duplicates if Woo triggers the hook twice
	$ran = true;
	...
	foreach ($blocked as $it) {
		$lines[] = sprintf(
			__('Minimum order for %1$s is %2$d units. You currently have %3$d.', 'your-td'),
			$it['name'],
			(int) $it['min'],
			(int) $it['qty']
		);
	}
	wc_add_notice(implode('<br>', array_map('esc_html', $lines)), 'error');
}, 20);
